...
import { unwrap } from '../src/utils.js'
...
describe('Workflow Bug Reproduction', () => {
    ...
    vi.spyOn(manager, 'connect').mockImplementation(async (_folder) => {
      ...
      return {
        session: {
          ...
          get: async (params: any) => { return sessionStore[params.path.id] },
          ...
          prompt: async (params: any) => {
            ...
            if (agent === 'plan') {
              if (text.includes('Analyze the context and produce a PRD')) {
                role = 'architect'
                callSequence.push('architect')
                responseText = JSON.stringify({
                  prd: "PRD content",
                  phases: [{ title: "Phase 1", description: "Do logic" }]
                })
  ...
  it('should run the full sequence: architect -> manager -> worker -> verifier', async () => {
     ...
     expect(archIdx).toBeLessThan(mgrIdx)
     expect(mgrIdx).toBeLessThan(wkrIdx) // In round 1
     expect(wkrIdx).toBeLessThan(vrfIdx)
