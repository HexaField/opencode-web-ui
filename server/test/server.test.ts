...
describe('Server Integration Tests', () => {
  ...
  it('should connect to a folder', async () => {
    const res = await request(app).post('/connect').send({ folder: tempDir })
  ...
  it('should list sessions (initially empty)', async () => {
    const res = await request(app).get(`/sessions?folder=${encodeURIComponent(tempDir)}`)
  ...
  it('should create and list agents', async () => {
    ...
    // List agents
    const listRes = await request(app).get(`/agents?folder=${encodeURIComponent(tempDir)}`)
  ...
  it('should get file status', async () => {
    const res = await request(app).get(`/files/status?folder=${encodeURIComponent(tempDir)}`)
  ...
  it('should read a file', async () => {
    const res = await request(app).get(`/files/read?folder=${encodeURIComponent(tempDir)}&path=test.txt`)
  ...
  it('should list files in a directory', async () => {
    const res = await request(app).get(`/fs/list?path=${encodeURIComponent(tempDir)}`)
  ...
  it('should get session details', async () => {
    ...
    const res = await request(app).get(`/sessions/${sessionId}?folder=${encodeURIComponent(tempDir)}`)
  ...
  it('should handle a multi-turn conversation', async () => {
    ...
    // 4. Verify history
    const sessionRes = await request(app).get(`/sessions/${sessionId}?folder=${encodeURIComponent(tempDir)}`)
