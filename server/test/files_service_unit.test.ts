...
vi.mock('child_process', () => {
  ...
  const execFile = vi.fn((_cmd, _args, opts, cb) => {
    if (typeof opts === 'function') {
      cb = opts
    }
    if (cb) cb(null, { stdout: '', stderr: '' })
...
describe('Files Service Unit Tests', () => {
  ...
  describe('GET /api/files/read', () => {
    it('should pass folder as directory to client.file.read', async () => {
      ...
      const res = await request(app).get('/api/files/read').query({ folder, path: filePath })
    ...
    it('should fallback to fs.readFile if SDK returns unknown format', async () => {
      ...
      const res = await request(app).get('/api/files/read').query({ folder: '/tmp', path: 'test.txt' })
  ...
  describe('GET /api/files/status', () => {
    it('should return file status from client', async () => {
      ...
      const res = await request(app).get('/api/files/status').query({ folder: '/tmp' })
  ...
  describe('GET /api/fs/list', () => {
    it('should list files and directories', async () => {
      ...
      const res = await request(app).get('/api/fs/list').query({ path: '/tmp' })
  ...
  describe('GET /api/fs/read', () => {
    it('should read file content from fs', async () => {
      ...
      const res = await request(app).get('/api/fs/read').query({ path: '/tmp/a.txt' })
  ...
  describe('GET /api/files/diff', () => {
    it('should return git diff for tracked file', async () => {
      ...
      const res = await request(app).get('/api/files/diff').query({ folder: '/tmp', path: 'file.txt' })
    ...
    it('should return isNew for untracked file', async () => {
      ...
      const res = await request(app).get('/api/files/diff').query({ folder: '/tmp', path: 'newfile.txt' })
  ...
  describe('GET /api/files/diff-summary', () => {
    it('should return diff summary', async () => {
      ...
      const res = await request(app).get('/api/files/diff-summary').query({ folder: '/tmp' })
